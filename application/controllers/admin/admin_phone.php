...
class admin_phone extends MY_Controller{
...
//		$this->load->view('auth/security/bind/mobile',$data);
	...
	public function upd_phone_show(){
		$data['user']=$this->admin_user_model->user_info();
		$this->load->view('admin/phone/shop_phone_1',$data);
	}
	//编辑新号码
...
//			echo "<script>alert('操作执行失败，请重试!');history.go(-1);</script>";
	...
	public function get_new_num(){
		...
		$where=array('shop_id'=>$_SESSION['admin_user']['shop_id']);
		$re=$this->admin_user_model->select('sys_shop',$where,$cont);
		if($re['0']['shop_mobile']==$shop_mobile){//原手机号码不符合
			echo "-1";
		}else{//验证成功发送短信
			//验证信息使用状态；
			$log_type=0;
...
public function test_email(){
	$this->send_email("caenzhang@foxmail.com","xxxxxxxxxxxxxxxxxxxxxxx","yyyyyyyyyyyyyyyyyyyy");
}
