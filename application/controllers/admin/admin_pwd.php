...
class admin_pwd extends MY_Controller{
	...
	public function upd_pwd_show(){
		$data['user']=$this->admin_user_model->user_info();
		$this->load->view('auth/security/passByLogin/update',$data);
	}
	
	//编辑新地址
	public function upd_pwd(){
		$bl=$this->admin_user_model->upd_pwd($_POST['shop_pwd_1']);
		if($bl){
			echo "<script>alert('密码修改成功，请重新登陆!');top.location='".site_url('admin/admin_login/login_out')."';</script>";
		}else{
