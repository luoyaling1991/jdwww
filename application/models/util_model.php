...
class Util_model extends MY_Model {
	...
	/**排序$tab_sort_id操作id的列名   $sort_id_1上一个的id   $sort_id_2下一个的id $sort_value自己的排序值
	 *$tab_name 要操作的表明  $tab_asc 要操作的排序字段   $asc_type 0置顶  1是上一  2是下一  3是置底
	*$where_arr查询条件数组  $upd_arr更改条件数组  $sort_value_1下一个值  $sort_value_2 上一个值
	* */
	function sort_do_1($tab_sort_id,$sort_value,$sort_id_1,$sort_id_2,$tab_name,$tab_asc,$asc_type,$where_arr,$upd_arr,$sort_value_1,$sort_value_2){
		if($asc_type==0){
			$max= $this->db->select("max({$tab_asc}) as max")->where($where_arr)->get($tab_name);
			$max_asc = $max->row_array();
			$max_asc=$max_asc['max']+1;
			...
			$data = array("{$tab_asc}" => $sort_value_1);
			$this->db->where(array("{$tab_sort_id}"=>$sort_id_1))->update($tab_name,array("{$tab_asc}"=>$sort_value));
		}else if($asc_type==2){
			$data = array("{$tab_asc}" => $sort_value_2);
			$this->db->where(array("{$tab_sort_id}"=>$sort_id_2))->update($tab_name,array("{$tab_asc}"=>$sort_value));
		}else if($asc_type==3){
			$min= $this->db->select("min({$tab_asc}) as min")->where($where_arr)->get($tab_name);
			$min_asc = $min->row_array();
			$min_asc=$min_asc['min']-1;
	...
	//删除推荐位
	function batch_do_2($tab_name,$tab_id_name,$type_value,$tab_id){
		foreach ($tab_id as $row){
				$this->db->where(array($tab_id_name=>$row,'show_type'=>$type_value))->delete($tab_name);
		}
	}
	//下架推荐位
	function batch_do_3($tab_name,$tab_id_name,$type_value,$tab_id){
		foreach ($tab_id as $row){
			$arr=array("show_state"=>0);
			$this->db->where(array($tab_id_name=>$row,'show_type'=>$type_value))->update($tab_name,$arr);
		}
	}
	/*删除
	 * 
	...
	//删除菜品、套餐关联的推荐位信息
	function delete_do_2($tab_name,$tab_id_name,$tab_id,$type_value){
		$this->db->where(array($tab_id_name=>$tab_id,'show_type'=>$type_value))->delete($tab_name);
	}
