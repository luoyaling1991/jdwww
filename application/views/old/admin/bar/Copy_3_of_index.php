...
<body class="page-header-fixed">
	...
	<?php 
		$del_img=constant('ADMIN_SRC')."media/img/btn_del.png";
		foreach ($tab_list as $one){
            $tab_one_list=$one['tab_list'];
			foreach ($tab_one_list as $row){
				$tab_id=$row['tab_id'];
				...
				if($tab_state==2){
					...
					echo "<div id='show_order_{$tab_id}' class='modal hide fade' tabindex='-1' data-replace='true' style='width:800px;'>
							<div class='modal-header' style='color:#FFF;background-color:#292929;height:30px;'>
							    <button type='button' class='close' data-dismiss='modal' aria-hidden='true'></button>
				                <h3 style='float:left;width:530px;'>{$type_name}{$tab_name}</h3>
				                <h5 style='margin-right:20px;float:right;'>就餐人数{$order_person}/{$tab_person}</h5>
							...
							echo "					<thead>
				                                    	<tr>
				                                        	<th colspan='6' style='background-color:#6EC169;'>
				                                            	<span style='float:left'>{$order_no_show}</span>
				                                                <span style='float:right'>{$insert_time}</span>
				                                            </th>
				                                        </tr>
							...
							foreach ($log_list as $log){
								...
								echo "
									<tr class='odd gradeX bg_color'>
											<td>$log_num_show</td>
											<td>$log_name</td>
                                            <td style='color:#F00;'>$log_price</td>
                                            <td>{$log_count}份</td>
											<td style='color:#F00;'>$log_money</td>
											<td>
					...
					echo "<div class='modal-footer' style='padding:0px;'>
				            	<table class='table table-striped table-bordered table-hover tab_border_no' id='sample_3'
				                	style='margin-bottom:0px;margin-left:25px;width:750px;'>
													<thead>
				                                    	<tr style='color:#FFF;background-color:#E02222;'>
				                                            ...
				                                            <th style='width:90px;'>{$order_sum}份</th>
				                                            <th>{$order_money}元</th>
				                                        </tr>
				                                     </thead>
				                </table>
